...
# ---------------- PAGE CONFIG ----------------
st.set_page_config(page_title="AgriIndia AI", layout="wide")

# ---------------- TITLE ----------------
st.title("🧠 AgriIndia – AI Models")
...
with tab4:
    ...
    # Use the same values stored in the trained encoders.
    # This prevents "previously unseen labels" errors during prediction.

    commodity_options = sorted(market_encoder["Commodity"].classes_)
    category_options = sorted(market_encoder["Category"].classes_)
    unit_options = sorted(market_encoder["Unit"].classes_)
    state_options = sorted(market_encoder["State"].classes_)
    season_options = sorted(market_encoder["Season"].classes_)
    ...
    # ---------------- District ----------------
    # Keep only districts that are both present in the dataset for the
    # selected state and known by the trained District encoder.

    encoded_districts = set(market_encoder["District"].classes_)
    ...
    district_list = sorted(
        ...
        ).intersection(encoded_districts)
    )

    if not district_list:
        st.error("❌ No compatible districts found for the selected state.")
        st.stop()
    ...
    # ---------------- Market ----------------
    # Keep only markets that are present for the selected district
    # and are known by the trained Market encoder.

    encoded_markets = set(market_encoder["Market"].classes_)

    market_list = sorted(
        set(
            market_data[
                market_data["District"] == district
            ]["Market"].dropna().unique()
        ).intersection(encoded_markets)
    )

    if not market_list:
        st.error("❌ No compatible markets found for the selected district.")
        st.stop()
    ...
    if st.button("💰 Predict Market Price"):
        ...
        if min_price == 0:
            st.error("❌ Minimum Price must be greater than 0.")
            st.stop()

        if average_price == 0:
            st.error("❌ Average Price must be greater than 0.")
            st.stop()
...
# =====================================================
st.markdown("---")
st.info("💬 AgriIndia Chatbot (Coming Soon)")